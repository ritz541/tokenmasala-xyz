...
function WeekdayBars({ accent, spend }: { accent: string; spend: number[] }) {
  ...
  const slot = WIDTH / WEEKDAY_LABELS.length;
  ...
  const activeTooltip =
    hovered === null
      ? null
      : (() => {
          const x = slot * hovered + (slot - barWidth) / 2;
          return {
  ...
  return (
    <div>
      <p className="text-4xl font-semibold tracking-tight">
        {peakIndex === null ? "—" : WEEKDAY_NAMES[peakIndex]}
      </p>
      <p className="mt-1 text-sm text-muted-foreground">Your peak day</p>
          ...
          {WEEKDAY_LABELS.map((label, index) => {
            ...
            return (
              <g key={`${label}-${index}`} onPointerEnter={() => setHovered(index)}>
                ...
                <rect fill="transparent" height={HEIGHT} width={slot} x={slot * index} y={0} />
                <rect
                  fill={accent}
                  height={height}
